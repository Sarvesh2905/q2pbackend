...
/* ─── Column alias normaliser ─────────────────────────────────────────────
   Maps any known Excel header variant → the internal field name used in
   validation & DB inserts. Add more aliases here as needed.
────────────────────────────────────────────────────────────────────────── */
const STANDARD_ALIAS = {
  // Customer part number
  Customer_PN:    "Customer_partno",
  CustomerPN:     "Customer_partno",
  customer_pn:    "Customer_partno",
  "Customer PN":  "Customer_partno",
  // CFTI part number
  CFTI_PN:        "Cfti_partno",
  CftiPN:         "Cfti_partno",
  cfti_pn:        "Cfti_partno",
  ...
  // SPL condition
  Spl_Condition:  "SPL_Cond",
  SPL_Condition:  "SPL_Cond",
  spl_condition:  "SPL_Cond",
  "SPL Condition":"SPL_Cond",
  Special_Condition: "SPL_Cond",
};

const normaliseRow = (raw, aliasMap) => {
  const out = {};
...
const createPrice = async (req, res) => {
  ...
  try {
    ...
    SPL_Cond        = SPL_Cond?.trim()  ? SPL_Cond.trim().substring(0, 20)  : null;
    Remarks         = Remarks?.trim()   ? Remarks.trim().substring(0, 30)   : null;
    ...
    await db.execute(
      ...
      [nextSno, Customer_partno, Cfti_partno, Description, price,
       Start_Date, Exp_Date, Curr, Leadtime, DeliveryTerm, SPL_Cond, Remarks, Product, Market],
    );
...
/* ── Standard Template — XLSX with exact internal headers ── */
const downloadStandardTemplate = (req, res) => {
  // Headers MUST match what uploadStandardPrices reads (after alias normalisation)
  // User fills these column names exactly — or the alias map will catch variants
  const headers = [
    ["Customer_partno", "Cfti_partno", "Description", "ListPrice",
     "Start_Date", "Exp_Date", "Curr", "Leadtime", "DeliveryTerm",
     "SPL_Cond", "Remarks", "Product", "Market", "status"],
  ...
  XLSX.utils.book_append_sheet(wb, ws, "StandardPrice");
  const buf = XLSX.write(wb, { bookType: "xlsx", type: "buffer" });

  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", 'attachment; filename="standard_price_template.xlsx"');
  return res.send(buf);
};

/* ── Standard Upload — with column alias normalisation ── */
const uploadStandardPrices = async (req, res) => {
  ...
  try {
    ...
    if (rawRows.length === 0)
      return res.json({ success: true, total: 0, inserted: 0, failed: 0, errors: [] });

    // ── Normalise column names using alias map ──────────────────────────
    const rows = rawRows.map((raw) => normaliseRow(raw, STANDARD_ALIAS));

    /* Fetch valid products from DB — only enforce if table is non-empty */
    ...
    rows.forEach((row, idx) => {
      ...
      const ltsaCode  = String(row.LTSA_Code       || "").trim();
      const custPN    = String(row.Customer_partno  || "").trim().toUpperCase();
      const cftiPN    = String(row.Cfti_partno      || "").trim().toUpperCase();
      const desc      = String(row.Description      || "").trim();
      const startStr  = toDateStr(row.Start_Date);
      const expStr    = toDateStr(row.Exp_Date);
      const curr      = String(row.Curr             || "").trim().toUpperCase();
      const leadtime  = String(row.Leadtime         || "").trim();
      const delivTerm = String(row.DeliveryTerm     || "").trim();
      const splCond   = String(row.SPL_Cond         || "").trim();
      ...
      if (!custPN || custPN.length < 1 || custPN.length > 10)
        errs.push(`Customer_partno must be 1–10 chars (got ${custPN.length})`);

      if (!cftiPN || cftiPN.length < 1 || cftiPN.length > 20)
        errs.push(`Cfti_partno must be 1–20 chars (got ${cftiPN.length})`);
      ...
      if (!startStr)
        ...
        errs.push(`Start_Date must be ≥ Jan 1 ${new Date().getFullYear()} (got ${startStr})`);

      if (!expStr)
        errs.push("Exp_Date is required and must be a valid date");
      ...
      if (!leadtime || leadtime.length > 15)
        errs.push("Leadtime is required and must be ≤ 15 chars");

      if (!delivTerm || delivTerm.length > 17)
        errs.push("DeliveryTerm is required and must be ≤ 17 chars");

      if (splCond.length > 20)
        errs.push("SPL_Cond must be ≤ 20 chars");

      if (remarks.length > 30)
        errs.push("Remarks must be ≤ 30 chars");

      if (!product)
        errs.push("Product is required");
      else if (product.length > 15)
        errs.push("Product must be ≤ 15 chars");
      

      if (!["FM", "AM"].includes(market))
        errs.push("Market must be FM or AM");
        ...
        await db.execute(
          `INSERT INTO price
             (Sno, LTSA_Code, Customer_partno, Cfti_partno, Description,
              ...
              SPL_Cond, Remarks, Product, Market, status)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [nextSno, row.ltsaCode, row.custPN, row.cftiPN, row.desc,
           row.listPrice, row.startStr, row.expStr, row.curr, row.leadtime,
           row.delivTerm, row.splCond, row.remarks, row.product, row.market, row.status],
        );
        inserted++;
      } catch (err) {
...
const createLtsaPrice = async (req, res) => {
  ...
  try {
    ...
    Customer_partno = String(Customer_partno).trim().toUpperCase().substring(0, 36);
    Cfti_partno     = String(Cfti_partno).trim().toUpperCase().substring(0, 18);
    Description     = Description?.trim() ? Description.trim().substring(0, 44) : null;
...
/* ── LTSA Template — XLSX with exact internal headers ── */
const downloadLtsaTemplate = (req, res) => {
  ...
  const ws = XLSX.utils.aoa_to_sheet(headers);
  ws["!cols"] = headers[0].map(() => ({ wch: 18 }));
  XLSX.utils.book_append_sheet(wb, ws, "LTSAPrice");
  const buf = XLSX.write(wb, { bookType: "xlsx", type: "buffer" });

  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", 'attachment; filename="ltsa_price_template.xlsx"');
  return res.send(buf);
};
...
const uploadLtsaPrices = async (req, res) => {
  ...
  try {
    ...
    rawRows.forEach((raw, idx) => {
      ...
      const ltsaCode  = String(raw.LTSA_Code       || "").trim().toUpperCase();
      const custPN    = String(raw.Customer_partno  || "").trim().toUpperCase();
      const cftiPN    = String(raw.Cfti_partno      || "").trim().toUpperCase();
      const desc      = String(raw.Description      || "").trim();
      const startStr  = toDateStr(raw.Start_Date);
      const expStr    = toDateStr(raw.Exp_Date);
      const curr      = String(raw.Curr             || "").trim().toUpperCase();
      const leadtime  = String(raw.Leadtime         || "").trim();
      const delivTerm = String(raw.DeliveryTerm     || "").trim();
      const product   = String(raw.Product          || "").trim().toUpperCase();
      const market    = String(raw.Market           || "").trim().toUpperCase();
      const status    = String(raw.status           || "Active").trim();

      if (!ltsaCode || ltsaCode.length !== 9)
        errs.push(`LTSA_Code must be exactly 9 chars (got "${ltsaCode}", length ${ltsaCode.length})`);

      if (custPN.length < 8 || custPN.length > 36)
        errs.push(`Customer_partno must be 8–36 chars (got ${custPN.length})`);

      if (cftiPN.length < 6 || cftiPN.length > 18)
        errs.push(`Cfti_partno must be 6–18 chars (got ${cftiPN.length})`);

      if (desc.length > 44)
        errs.push(`Description exceeds 44 chars (got ${desc.length})`);

      const splPriceNum = Number(raw.SplPrice);
      if (!Number.isInteger(splPriceNum) || splPriceNum <= 0 || splPriceNum > 99999)
        errs.push("SplPrice must be a positive integer ≤ 99999");

      if (!startStr)
        errs.push("Start_Date is required and must be a valid date");
      else if (startStr < jan1Str)
        errs.push(`Start_Date must be ≥ Jan 1 ${today.getFullYear()} (got ${startStr})`);
      ...
      if (!expStr)
        ...
        errs.push(`Exp_Date must be ≤ Dec 31 ${today.getFullYear()} (got ${expStr})`);

      if (!curr || curr.length > 3)
        errs.push("Curr is required and must be ≤ 3 chars");

      if (leadtime.length > 15)
        errs.push("Leadtime must be ≤ 15 chars");

      if (delivTerm.length > 16)
        errs.push("DeliveryTerm must be ≤ 16 chars");
    ...
    for (const row of validRows) {
      try {
        const [existing] = await db.execute(
          "SELECT Sno FROM ltsa_price WHERE LTSA_Code = ?", [row.ltsaCode],
        );
        if (existing.length > 0) {
          dbErrors.push({
        ...
        await db.execute(
          `INSERT INTO ltsa_price
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [nextSno, row.ltsaCode, row.custPN, row.cftiPN, row.desc,
           row.splPrice, row.startStr, row.expStr, row.curr, row.leadtime,
           row.delivTerm, row.product, row.market, row.status],
        );
        inserted++;
      } catch (err) {
        dbErrors.push({
...
module.exports = {
  listPrices, createPrice, updatePrice, togglePriceStatus,
  downloadPrices, downloadStandardTemplate, uploadStandardPrices,
  listLtsaPrices, createLtsaPrice, updateLtsaPrice, toggleLtsaPriceStatus,
  downloadLtsaPrices, downloadLtsaTemplate, uploadLtsaPrices,
  getNextLtsaCode, checkLtsaCodeExists,
};