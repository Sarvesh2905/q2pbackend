...
const getFacingFactories = async (req, res) => {
  try {
    const [rows] = await db.execute(
      `SELECT Data FROM quote_data WHERE Type = 'Facingfactory' AND Status = 'Active' ORDER BY Data ASC`
    );
    return res.json({ data: rows.map((r) => r.Data) });
  } catch (err) {
    return res.status(500).json({ message: err.message });
...
const addOpportunityType = async (req, res) => {
  ...
  try {
    ...
    const [existing] = await db.execute(
      `SELECT Sno FROM quote_data WHERE Data = ? AND Type = 'Opportunitytype'`, [val]
    );
    if (existing.length) return res.status(400).json({ message: "Already exists" });
...
const addFacingFactory = async (req, res) => {
  ...
  try {
    const val = value.trim().toUpperCase();
    const [existing] = await db.execute(
      `SELECT Sno FROM quote_data WHERE Data = ? AND Type = 'Facingfactory'`, [val]
    );
    if (existing.length) return res.status(400).json({ message: "Already exists" });
    const [maxRow] = await db.execute(`SELECT MAX(Sno) AS m FROM quote_data`);
    const nextSno = (maxRow[0].m || 0) + 1;
...
const createEnquiry = async (req, res) => {
  const {
    ...
    group_name, currency, end_user_name, end_country, end_industry,
    receipt_date, ae_name, sales_contact, opportunity_type,
    rfq_category, rfq_reference, comments,
    facing_factory, products, project_name,
    customer_due_date, proposed_due_date, lines_in_rfq, win_probability,
    quote_number, opportunity_stage, expected_order_date, eff_enq_date, priority,
  } = req.body;
  ...
  try {
    ...
    const [[maxRow]] = await conn.execute(`SELECT MAX(Sno) AS m FROM quote_register`);
    ...
    await conn.execute(
      ...
      [
        nextSno, today, sales_contact, ae_name,
        customer_name, customer_type || null, customer_country || null,
        buyer_name || null, group_name || null, opportunity_type, project_name,
        end_user_name, end_country || null, end_industry || null,
        rfq_reference || null, receipt_date, rfq_category,
        eff_enq_date || null, customer_due_date, proposed_due_date,
        quote_number, facing_factory, productStr,
        parseInt(lines_in_rfq), currency || null, win_probability,
        opportunity_stage, comments || null, expected_order_date || null, priority,
      ]
    );

    const productList = Array.isArray(products) ? products : [products];
    for (const product of productList) {
      const [[tMaxRow]] = await conn.execute(`SELECT MAX(Sno) AS m FROM quote_timeline`);
      const tNextSno = (tMaxRow.m || 0) + 1;
      await conn.execute(
        `INSERT INTO quote_timeline (Sno, Quote_number, Dept_user, RFQ_Date, Product) VALUES (?,?,?,?,?)`,
        [tNextSno, quote_number, ae_name, receipt_date, product]
      );
    }

    await conn.commit();
    return res.json({ success: true, message: "Enquiry created successfully!", quote_number });
  } catch (err) {
    await conn.rollback();
...
const listEnquiries = async (req, res) => {
  try {
    const {
      ae, sales, product, quote_stage, opp_stage,
      priority, search, date_from, date_to,
    } = req.query;

    const where  = [];
    const params = [];

    if (ae)          { where.push("Dept_user = ?");          params.push(ae); }
    if (sales)       { where.push("Sales_contact = ?");      params.push(sales); }
    if (product)     { where.push("Product LIKE ?");         params.push(`%${product}%`); }
    if (quote_stage) { where.push("Quote_stage = ?");        params.push(quote_stage); }
    if (opp_stage)   { where.push("Opportunity_stage = ?");  params.push(opp_stage); }
    if (priority)    { where.push("Priority = ?");           params.push(priority); }
    if (date_from)   { where.push("RFQ_REG_Date >= ?");      params.push(date_from); }
    if (date_to)     { where.push("RFQ_REG_Date <= ?");      params.push(date_to); }

    if (search) {
      where.push(`(Quote_number LIKE ? OR Customer_name LIKE ? OR
...
const getRegisterFilterOptions = async (req, res) => {
  try {
    const [[aes], [sales], [stages], [opp], [products]] = await Promise.all([
      db.execute("SELECT DISTINCT Dept_user AS val FROM quote_register WHERE Dept_user IS NOT NULL ORDER BY Dept_user"),
      db.execute("SELECT DISTINCT Sales_contact AS val FROM quote_register WHERE Sales_contact IS NOT NULL ORDER BY Sales_contact"),
      db.execute("SELECT DISTINCT Quote_stage AS val FROM quote_register WHERE Quote_stage IS NOT NULL ORDER BY Quote_stage"),
      db.execute("SELECT DISTINCT Opportunity_stage AS val FROM quote_register WHERE Opportunity_stage IS NOT NULL ORDER BY Opportunity_stage"),
      db.execute("SELECT DISTINCT Product AS val FROM quote_register WHERE Product IS NOT NULL ORDER BY Product"),
    ]);
    return res.json({
      ae:          aes.map((r) => r.val),
...
const downloadEnquiries = async (req, res) => {
  try {
    ...
    const fields = [
      "Sno", "ae_name", "Sales_contact", "Quote_number", "quote_date",
      "price_k", "Customer_name", "End_user_name", "Product", "Project_name",
      "cust_due_date", "probability", "Quote_stage", "category", "opp_stage",
      "Rev", "Priority",
    ];

    res.header("Content-Type", "text/csv");
    res.header("Content-Disposition", 'attachment; filename="enquiry_register.csv"');
