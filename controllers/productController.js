...
const checkProductExists = async (req, res) => {
  ...
  try {
    if (Products) {
      ...
      if (rows.length > 0)
        return res.json({ exists: true, field: 'Products', message: 'Product already exists.' });
    }
...
const createProduct = async (req, res) => {
  ...
  if (!Products || !Facing_Factory || !Prd_group)
    return res.status(400).json({ message: 'Product, Facing Factory and Group are required.' });
...
// PUT /api/products/:sno — locked: Products, Prd_group; editable: Description, Facing_Factory
const updateProduct = async (req, res) => {
  ...
  Description    = Description    && Description.trim()    !== '' ? Description.trim()    : null;
  Facing_Factory = Facing_Factory && Facing_Factory.trim() !== '' ? Facing_Factory.trim().toUpperCase() : null;

  try {
    const [rows] = await db.execute('SELECT * FROM product WHERE Sno = ?', [sno]);
...
const toggleProductStatus = async (req, res) => {
  ...
  const sno = parseInt(req.params.sno, 10);
  try {
    const [rows] = await db.execute('SELECT status FROM product WHERE Sno = ?', [sno]);
    if (rows.length === 0) return res.status(404).json({ message: 'Not found' });
    const next = rows[0].status === 'Active' ? 'Inactive' : 'Active';
    await db.execute('UPDATE product SET status = ? WHERE Sno = ?', [next, sno]);
